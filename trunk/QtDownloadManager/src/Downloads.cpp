...
//	ReadParameterToDownload();
	...
	}else if(DL_Downloader.contains("wget")){
//		qDebug("came here for wget ");
		proc = new QProcess();
		QString command = QString("%1/scripts/%2").arg(QApplication::applicationDirPath()).arg("/wgethelper.sh");
		QStringList arguments;
		arguments << DL_Url;
		proc->setWorkingDirectory(DL_SaveDir);
		proc->start(command,arguments);
...
//				Saving to: `ubuntu-9.04-desktop-i386.iso'
				DL_Download =line.remove("Saving to: ").remove("`").remove("'");
				localmodel->setData(localmodel->index(localrow,0),DL_Download);
			}else if(line.contains("Length: ")){
				DL_TotalSize = line.remove("Length: ").split(" ").at(0);
				localmodel->setData(localmodel->index(localrow,2),DL_TotalSize);
			}else if(line.contains("%")){
				QStringList linesplitlist = line.split(" ",QString::SkipEmptyParts);
				if(linesplitlist.size()>=9){
//					qDebug("came to here");
					DL_Status = QString("%1(running)").arg(linesplitlist.at(6));
					QStandardItem *item = new QStandardItem(DL_Status);
					item->setBackground(QColor("#6ee070"));
					item->setForeground(QColor("#000000"));
					localmodel->setItem(localrow, 1, item);
//					emit updateinterface();
					if(linesplitlist.at(7).contains("."))
						DL_Speed = QString("%1B/s").arg(linesplitlist.at(7));
					localmodel->setData(localmodel->index(localrow,3),DL_Speed);
				}
